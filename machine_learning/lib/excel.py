...
class Excel(object):
    ...
    def load(self, file_name, title_map):
        '''
        ...
        work_movie = xlrd.open_workbook(file_name)
        sheet = work_movie.sheet_by_index(0)
        for c in xrange(sheet.ncols):
            title_list.append(sheet.cell(0, c).value)
        for r in xrange(1, sheet.nrows):
            unit = {}
            for c in xrange(sheet.ncols):
                title = title_list[c].encode("utf-8")
                if title not in title_map:
                    continue
                data = sheet.cell(r, c).value
                if isinstance(data, unicode):
                    unit[title_map[title]] = data.encode("utf-8")
                else:
                    unit[title_map[title]] = data
